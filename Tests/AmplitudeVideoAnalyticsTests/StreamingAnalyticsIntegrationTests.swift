...
final class StreamingAnalyticsIntegrationTests: XCTestCase {
    func testViewSessionEndToEnd() {
        ...
        player.duration = 100
        let session = plugin.trackVideo(player: player, options: VideoTrackingOptions(contentId: "ep-1", deliveryMode: .onDemand))
        player.fire(.played)
        ...
        XCTAssertEqual(stopped.eventProperties?["stream_session_id"] as? String, session.id)

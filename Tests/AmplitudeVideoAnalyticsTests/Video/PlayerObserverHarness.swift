...
final class PlayerObserverHarness {
    ...
    init(label: String, duration: TimeInterval? = 100, started: Bool = true, pulseInterval: TimeInterval = 3600) {
        ...
        // A real `PulseTimer`, so the timing tests still tick on their own, plus a handle on its handler so the
        // rest can tick by hand instead of racing a clock.
        observer = PlayerObserver(player: player, queue: queue, logger: logger) { state in
            recorder.record(state)
            reaction.hook?(state)
        } makePulse: { tick in
            ticker.tick = tick
            return PulseTimer(interval: pulseInterval, queue: queue, handler: tick)
        }
    ...
    /// Fires the observer's pulse handler as its timer would. Must run on `queue`. Before the first commit there
    /// is no timer yet — and no open play for a tick to book either, so firing nothing is the same outcome.
    ...
    func refresh() { onQueue { tick() }; drain() }
    ...
    /// Written when the observer builds its timer and read when a test ticks — both on `queue`.

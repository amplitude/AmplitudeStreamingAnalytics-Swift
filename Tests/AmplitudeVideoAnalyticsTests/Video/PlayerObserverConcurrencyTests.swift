...
final class PlayerObserverConcurrencyTests: XCTestCase {
    ...
    func testConcurrentPlayerEventsAndFinishFinalizeExactlyOnce() {
        for iteration in 0..<200 {
            ...
            for worker in 0..<6 {
                DispatchQueue.global().async(group: group) {
                    for step in 0..<20 {
                        ...
                        case 2: harness.player.fire(.seekStarted)
                        case 3: harness.queue.async { harness.observer.refresh() }
            ...
            XCTAssertEqual(harness.observer.state.phase, .final)
        }
    }

    /// `onChange` fires only after `commit` already updated `state` and returned; a `finish()` called from
    /// there still reaches `.final` exactly once.

...
final class StreamingEmitterTransportTests: XCTestCase {
    ...
    func testAPlayAndPauseFinalizeTheSnapshotRowOnTheWire() throws {
        ...
        // Read on the test's thread; the emitter itself is only touched inside the observer's callback.
        var emitter = StreamingEventEmitter(options: VideoTrackingOptions(contentId: "ep-1"))
        let streamSessionId = emitter.streamSessionId
        let at = Date(timeIntervalSince1970: 1_752_000_000)
        harness.onEveryState { state in
            let (next, events) = emitter.events(for: state, at: at)
            emitter = next

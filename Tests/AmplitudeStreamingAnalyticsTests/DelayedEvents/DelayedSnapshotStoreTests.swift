...
final class DelayedSnapshotStoreTests: XCTestCase {
    ...
    private func entry(insertId: String, revision: Int = 0) -> DelayedEntry {
        let event = BaseEvent(eventType: "Video Content Stopped")
        event.insertId = insertId
        return DelayedEntry(event: event, revision: revision)
    }
    ...
    func testSaveLoadRoundTripAcrossMultipleDelayIds() {
        ...
        let saved = DelayedStore(states: [
            "delay-a": DelayedState(
                entries: ["ins-1": entry(insertId: "ins-1", revision: 2)],
                pendingInstantEvents: []
            ),
            "delay-b": DelayedState(
                entries: ["ins-2": entry(insertId: "ins-2")],
        ...
        XCTAssertEqual(loaded?.states["delay-a"]?.entries["ins-1"]?.event.insertId, "ins-1")
        ...
        XCTAssertEqual(loaded?.states["delay-b"]?.entries["ins-2"]?.revision, 0)
    ...
    private func nonEmptyStore(delayId: String = "d") -> DelayedStore {
        DelayedStore(states: [delayId: DelayedState(
            entries: ["ins-1": entry(insertId: "ins-1")],
            pendingInstantEvents: []
        )])
    }
    ...
    func testUndecodableFileIsDiscarded() {
        ...
        XCTAssertFalse(persistedFileExists())
    }
    ...
    func testStorageDirectoryIsExcludedFromBackup() throws {
        ...
        XCTAssertTrue(waitForBackupExclusion(), "directory was never excluded from backup")
    }

    /// macOS applies the exclusion behind `backupd`, so it lands a little after `persist` returns.
    /// Each read needs its own URL with the cache dropped: resource values are cached by path, so
    /// a fresh instance still answers with the value cleared above.
    private func waitForBackupExclusion(timeout: TimeInterval = 2) -> Bool {
        let deadline = Date().addingTimeInterval(timeout)
        repeat {
            var directory = fileUrl().deletingLastPathComponent()
            directory.removeAllCachedResourceValues()
            let values = try? directory.resourceValues(forKeys: [.isExcludedFromBackupKey])
            if values?.isExcludedFromBackup == true { return true }
    ...
    func testLoadIsNilBeforeAnyPersist() {
        XCTAssertFalse(persistedFileExists())
    ...
    func testEnrichedSnapshotEncodedSize() throws {
        ...
        let entry = DelayedEntry(event: event, revision: 1)

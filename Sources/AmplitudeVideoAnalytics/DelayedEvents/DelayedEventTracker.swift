...
final class DelayedEventTracker {
    ...
    func flush() {
        queue.async {
            // Entries stay local until the request settles; suspend the pulse and cancel any
            // pending coalesced send so neither re-upserts the row the server is deleting.
            self.timer.suspend()
            self.sendScheduled = false
            self.send(flushing: true)
        }
    }
    ...
    /// `sendScheduled`'s two roles: while set, adds before the hop share the pending request; `flush()` clears it to cancel the hop.
    private func scheduleSend() {
        guard !sendScheduled else { return }
        sendScheduled = true
        queue.async {
            guard self.sendScheduled else { return }
            self.sendScheduled = false
            self.send()
        }
    }

    private func send(flushing: Bool = false) {
        ...
        httpClient.upload(body) { [weak self] result in
            ...
            guard !settledIds.isEmpty else { return }
            self.queue.async {
